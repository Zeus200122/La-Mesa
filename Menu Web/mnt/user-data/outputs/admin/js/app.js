...
async function init() {
  ...
  db.auth.onAuthStateChange((event, session) => {
    if (event === 'SIGNED_IN')  mostrarPanel()
...
function mostrarLogin() {
  ...
  elAdminPanel.hidden  = true
}

async function mostrarPanel() {
  elLoginScreen.hidden = true
  elAdminPanel.hidden  = false
...
async function login() {
  ...
  if (!email || !pass) { elLoginError.textContent = 'Completa todos los campos.'; return }
...
async function guardarProducto() {
  ...
  try {
    ...
    console.error(err)
    elFormError.textContent = 'Error al guardar. Revisa los datos e intenta de nuevo.'
  } finally {
